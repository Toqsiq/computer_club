    ...
    with get_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("""
                ...
                ('Ночная смена', 480, 900.00, 'Ночной тариф с 00:00 до 08:00')
                ON CONFLICT DO NOTHING;
            """)
            ...
            cur.execute("""
                ...
                ('Кофе', 100.00, 'Горячий кофе'),
                ('Энергетик', 120.00, 'Энергетический напиток'),
                ('Аренда наушников', 50.00, 'Наушники на время сессии'),
                ('VIP кресло', 80.00, 'Улучшенное кресло'),
                ('Бутерброд', 90.00, 'Сытный бутерброд')
                ON CONFLICT DO NOTHING;
            """)
